...
public class DeviceManagerController {
    ...
    private com.xtc.sync.cft httpClient;
    ...
    private java.util.Map<String, String> httpResultCodeMap;
    private java.util.Map<String, String> serverCodeResultMap;
    ...
    private ObservableList<String> selectedParameters = FXCollections.observableArrayList();
    ...
    public void initialize() {
        ...
        httpClient = new com.xtc.sync.cft(null);
            ...
            if (event.getClickCount() == 1) {
                ...
                if (selectedRequest != null) {
                    ...
                    if (selectedScript != null) {
                        editRequestInDialog(null, selectedScript, null, selectedRequest);
                    }
                }
            }
        });
    ...
    private void setupRealTimeParamUpdate() {
        ...
        TextField[] paramFields = {
            versionNameField, versionCodeField, urlField
        };
    ...
    private void loadPackages() {
        ...
        try {
            ...
            java.nio.file.Path packagePath = DataPathManager.getPackagesJsonPath();
            if (!java.nio.file.Files.exists(packagePath)) {
                // 如果data文件夹中的文件不存在，从资源文件创建默认配置
                log("提示: packages.json 不存在，从默认配置创建");
                try {
                    String defaultJsonStr = new String(java.nio.file.Files.readAllBytes(java.nio.file.Paths.get(getClass().getResource(DataPathManager.getDefaultPackagesResourcePath()).toURI())), java.nio.charset.StandardCharsets.UTF_8);
                    DataPathManager.ensureDataDirExists();
                    java.nio.file.Files.write(packagePath, defaultJsonStr.getBytes(java.nio.charset.StandardCharsets.UTF_8));
                    log("已创建 packages.json");
                } catch (Exception e) {
                    log("警告: 无法从默认配置创建文件 - " + e.getMessage());
                    // 如果创建失败，直接使用资源文件
                    String jsonStr = new String(java.nio.file.Files.readAllBytes(java.nio.file.Paths.get(getClass().getResource(DataPathManager.getDefaultPackagesResourcePath()).toURI())), java.nio.charset.StandardCharsets.UTF_8);
            ...
            String jsonStr = new String(java.nio.file.Files.readAllBytes(packagePath), java.nio.charset.StandardCharsets.UTF_8);
            org.json.JSONArray jsonArray = new org.json.JSONArray(jsonStr);
            ...
            log("警告: 无法加载packages.json - " + e.getMessage());
        }
    }
    ...
    private void loadHttpResultCodes() {
        try {
            String jsonStr = new String(java.nio.file.Files.readAllBytes(java.nio.file.Paths.get(getClass().getResource("/http_result.json").toURI())), java.nio.charset.StandardCharsets.UTF_8);
            ...
            log("警告: 无法加载http_result.json - " + e.getMessage());
            httpResultCodeMap = new java.util.HashMap<>();
        }
    }
    ...
    private void loadServerCodeResultCodes() {
        try {
            String jsonStr = new String(java.nio.file.Files.readAllBytes(java.nio.file.Paths.get(getClass().getResource("/server_code_result.json").toURI())), java.nio.charset.StandardCharsets.UTF_8);
            org.json.JSONObject json = new org.json.JSONObject(jsonStr);
            ...
            log("警告: 无法加载server_code_result.json - " + e.getMessage());
            serverCodeResultMap = new java.util.HashMap<>();
        }
    }
    ...
    private void onAddFromADB() {
        ...
        if (result.isPresent()) {
            ...
            log("正在从设备导出上下文: " + deviceId);

            // 显示进度指示器
            showAlert(Alert.AlertType.INFORMATION, "正在导出", "正在从设备导出上下文，请稍候...");
    ...
    private void onAddFromJson() {
        ...
        if (result.isPresent()) {
            ...
            // 显示进度指示器
            showAlert(Alert.AlertType.INFORMATION, "正在解析", "正在解析JSON配置，请稍候...");
    ...
    private void onSwitchDevice() {
        ...
        log("已切换到设备: " + selected.getDeviceName() + " (" + bindNumber + ")");
    }
    ...
    private void onRenameDevice() {
        ...
        if (result.isPresent()) {
            ...
            if (!newName.isEmpty()) {
                ...
                log("设备重命名: " + selected.getDeviceName() + " -> " + newName);
            }
        }
    }
    ...
    private void onRemoveDevice() {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            log("设备删除: " + selected.getDeviceName());
        }
    }
    ...
    private void onExportDevice() {
        ...
        if (file != null) {
            try {
                ...
                log("设备配置导出: " + file.getAbsolutePath());
            } catch (Exception e) {
                showAlert(Alert.AlertType.ERROR, "错误", "导出失败: " + e.getMessage());
                log("错误: 导出失败 - " + e.getMessage());
                e.printStackTrace();
            }
        }
    }
    ...
    private void onSendRequest() {
            ...
            if (context != null) {
                ...
                if (!versionCodeText.isEmpty()) {
                    try {
                        ...
                        log("警告: VersionCode格式错误");
                    }
                }
                log("设置应用信息: " + selectedPackage.getPackageName());
            }
        }
        ...
        if (!selectedParameters.isEmpty()) {
            ...
            for (String paramDisplay : selectedParameters) {
                String paramName = extractParameterName(paramDisplay.split(" -> ")[0]);
                if (paramName != null) {
                    parameterNames.add(paramName);
                }
            }
            
            formattedBody = paramManager.formatRequestBodyWithParameters(body, current, parameterNames);
            log("请求体包含参数，已格式化");
        }
        
        log("发送HTTP请求: " + method + " " + url);
        log("原始请求体: \n" + body);
        if (!formattedBody.equals(body)) {
            log("格式化后请求体: \n" + formattedBody);
        }
        ...
        try {
            ...
            if (method.equals("POST")) {
                ...
                showAlert(Alert.AlertType.ERROR, "错误", "不支持的请求方法: " + method);
                return;
            }
    ...
    private void loadScripts() {
        try {
            java.nio.file.Path scriptPath = DataPathManager.getHttpScriptsJsonPath();
            if (!java.nio.file.Files.exists(scriptPath)) {
                log("提示: http_scripts.json 不存在，从默认配置创建");
                try {
                    String defaultJsonStr = new String(java.nio.file.Files.readAllBytes(java.nio.file.Paths.get(getClass().getResource(DataPathManager.getDefaultScriptsResourcePath()).toURI())), java.nio.charset.StandardCharsets.UTF_8);
                    DataPathManager.ensureDataDirExists();
                    java.nio.file.Files.write(scriptPath, defaultJsonStr.getBytes(java.nio.charset.StandardCharsets.UTF_8));
                    log("已创建 http_scripts.json");
                } catch (Exception e) {
                    log("警告: 无法从default_scripts.json创建文件 - " + e.getMessage());
                    return;
                }
            }
            String jsonStr = new String(java.nio.file.Files.readAllBytes(scriptPath), java.nio.charset.StandardCharsets.UTF_8);
            org.json.JSONArray jsonArray = new org.json.JSONArray(jsonStr);
            scriptList.clear();
            ...
            log("警告: 无法加载http_scripts.json - " + e.getMessage());
        }
    }
    ...
    private void saveScripts() {
        try {
            ...
            DataPathManager.ensureDataDirExists();
            java.nio.file.Files.write(DataPathManager.getHttpScriptsJsonPath(), jsonStr.getBytes(java.nio.charset.StandardCharsets.UTF_8));
        } catch (Exception e) {
            log("错误: 保存脚本失败 - " + e.getMessage());
        }
    }
    ...
    private void onParameterSelected() {
        String selected = parameterCombo.getSelectionModel().getSelectedItem();
        if (selected == null || selected.startsWith("---") || selected.equals("选择参数")) {
            return;
        }
        
        log("选择的参数显示文本: " + selected);
        
        // 从显示文本中提取参数名
        String paramName = extractParameterName(selected);
        if (paramName == null) {
            log("无法解析参数名，显示文本: " + selected);
            showAlert(Alert.AlertType.WARNING, "警告", "无法解析参数名: " + selected);
            return;
        }
        
        // 直接添加到已选参数列表（使用位置参数格式）
        ...
        log("已添加参数: " + paramName + " (位置: %" + parameterIndex + "$s)");
    }
    ...
    private void onGenerateTemplate() {
        ...
        for (int i = 0; i < selectedParameters.size(); i++) {
            String paramDisplay = selectedParameters.get(i);
            String paramName = extractParameterName(paramDisplay.split(" -> ")[0]);
        ...
        if (requestBodyArea != null) {
            ...
            log("已生成格式化模板，使用位置参数: " + paramNames);
        }
    }
    ...
    private void onAddRequest() {
        ...
        HBox parameterButtons = new HBox(5);
        ...
        scriptParamCombo.setOnAction(event -> {
            String selected = scriptParamCombo.getSelectionModel().getSelectedItem();
            if (selected == null || selected.equals("选择参数")) {
                return;
            }
        ...
        generateButton.setOnAction(event -> {
            ...
            for (int i = 0; i < scriptSelectedParameters.size(); i++) {
                String paramDisplay = scriptSelectedParameters.get(i);
                String paramName = extractParameterName(paramDisplay.split(" -> ")[0]);
                
                templateBuilder.append("    \"").append(paramName).append("\": \"%")
                              .append(i + 1).append("$s\"");
                
                if (i < scriptSelectedParameters.size() - 1) {
                    templateBuilder.append(",\n");
                } else {
                    templateBuilder.append("\n");
                }
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == addButtonType) {
                ...
                // 保存参数列表
                java.util.List<String> paramsToSave = ParameterDisplayHelper.fromDisplayList(scriptSelectedParameters);
                request.setParameters(paramsToSave);
        ...
        result.ifPresent(request -> {
            ...
            saveScripts();
            log("请求添加成功: " + request.getMethod() + " " + request.getUrl());
        });
    }
    ...
    private void onAddPackageConfig() {
        ...
        result.ifPresent(packageInfo -> {
            try {
                ...
                DataPathManager.ensureDataDirExists();
                java.nio.file.Files.write(packagePath, jsonArray.toString(2).getBytes(java.nio.charset.StandardCharsets.UTF_8));
                
                loadPackages();
                log("应用配置添加成功: " + packageInfo.getPackageName());
            } catch (Exception e) {
    ...
    private void createScriptExecutionDialog(HttpScript script, DeviceInfo device) {
        ...
        Label requestListLabel = new Label("请求列表 (双击编辑):");
        ...
        new Thread(() -> {
            ...
            for (int i = 0; i < script.getRequests().size() && shouldContinue[0]; i++) {
                ...
                Platform.runLater(() -> {
                    statusLabel.setText("执行中: " + requestIndex + "/" + totalRequests + " - " + request.getMethod() + " " + request.getUrl());
                    logArea.appendText("\n[" + requestIndex + "/" + totalRequests + "] " + request.getMethod() + " " + request.getUrl() + "\n");
                });

                try {
                    ContextManager context = device.getContext();
                    if (context != null && request.getPackageName() != null && !request.getPackageName().isEmpty()) {
                        ...
                        Platform.runLater(() -> logArea.appendText("设置应用包名: " + request.getPackageName() + 
                            (versionCode != null ? " (版本号: " + versionCode + ")" : "") +
                            (versionName != null && !versionName.isEmpty() ? " (版本名称: " + versionName + ")" : "") + "\n"));
                    }

                    // 应用参数格式化
                    String formattedBody = request.getBody();
                    if (request.hasParameters()) {
                        formattedBody = request.getFormattedBody(device);
                        final String finalFormattedBody = formattedBody;
                        Platform.runLater(() -> {
                            logArea.appendText("请求体包含" + request.getParameters().size() + "个参数\n");
                            logArea.appendText("格式化后请求体: " + (finalFormattedBody.length() > 100 ? finalFormattedBody.substring(0, 100) + "..." : finalFormattedBody) + "\n");
                        });
                    }
                    ...
                    if (request.getMethod().equals("POST")) {
                        ...
                        Platform.runLater(() -> {
                            logArea.appendText("错误: 不支持的请求方法: " + request.getMethod() + "\n");
    ...
    private void editRequestInDialog(Stage parentDialog, HttpScript script, ListView<HttpRequest> requestListView, HttpRequest request) {
        Dialog<HttpRequest> dialog = new Dialog<>();
        ...
        if (request.getParameters() != null && !request.getParameters().isEmpty()) {
            editSelectedParameters.addAll(ParameterDisplayHelper.toDisplayList(request.getParameters()));
        }
        ...
        editParamCombo.setOnAction(event -> {
            String selected = editParamCombo.getSelectionModel().getSelectedItem();
            if (selected == null || selected.equals("选择参数")) {
                return;
            }
        ...
        generateButton.setOnAction(event -> {
            ...
            for (int i = 0; i < editSelectedParameters.size(); i++) {
                String paramDisplay = editSelectedParameters.get(i);
                String paramName = extractParameterName(paramDisplay.split(" -> ")[0]);
                
                templateBuilder.append("    \"").append(paramName).append("\": \"%")
                              .append(i + 1).append("$s\"");
                
                if (i < editSelectedParameters.size() - 1) {
                    templateBuilder.append(",\n");
                } else {
                    templateBuilder.append("\n");
                }
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == saveButtonType) {
                ...
                // 保存参数列表
                java.util.List<String> paramsToSave = ParameterDisplayHelper.fromDisplayList(editSelectedParameters);
                request.setParameters(paramsToSave);
        ...
        result.ifPresent(updatedRequest -> {
            ...
            if (index >= 0) {
                ...
                requestListView.setItems(FXCollections.observableArrayList(script.getRequests()));
                saveScripts();
                log("请求已更新: " + updatedRequest.getMethod() + " " + updatedRequest.getUrl());
            }
        });
    }
